...
ColVis.prototype = {
	...
	"_fnDrawCallback": function ()
	{
		...
		for ( var i=0, iLen=aoColumns.length ; i<iLen ; i++ )
		{
			if ( this.dom.buttons[i] !== null )
			{
				...
				{
                    $('input', this.dom.buttons[i])[0].checked = true;
					//$('input', this.dom.buttons[i]).attr('checked','checked');
				}
				else
				{
                    $('input', this.dom.buttons[i])[0].checked = false;
	...
	"_fnDomShowAllButton": function ()
	{
		...
		$(nButton).click( function (e) {
			for ( var i=0, iLen=that.s.abOriginal.length ; i<iLen ; i++ )
			{
				if (jQuery.inArray(i,that.s.aiExclude) === -1)
				{
					that.s.dt.oInstance.fnSetColumnVis( i, true, false );
				}
				if ( that.s.fnStateChange !== null )
        			{
            			that.s.fnStateChange.call( that, i, true );
        			}
			}
			that._fnAdjustOpenRows();
			that.s.dt.oInstance.fnAdjustColumnSizing( false );
	...
	"_fnDomColumnButton": function ( i )
	{
		...
		$(nButton).click( function (e) {
			var showHide = !$('input', this)[0].checked;

			if ( e.target.nodeName.toLowerCase() == "input" )
			{
				showHide = $('input', this)[0].checked;
			}
	...
	"_fnCollectionShow": function ()
	{
		...
		$(".ColVis_Button.ColVis_MasterButton i").attr("class","icon-remove");
		if ( !this.s.bCssPosition )
		{
	...
	"_fnCollectionHide": function (  )
	{
		...
		$(".ColVis_Button.ColVis_MasterButton i").attr("class","icon-chevron-down");
	},
