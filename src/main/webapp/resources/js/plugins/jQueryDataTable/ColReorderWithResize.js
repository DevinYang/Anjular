...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo )
{
	...
	for ( i=0, iLen=iCols ; i<iLen ; i++ )
	{
		...
		//for datatable with filter row in header sort bind element for the first one
		if(oSettings.aoColumns[i].nTh.children.length > 1){
			$(oSettings.aoColumns[i].nTh.children[0]).unbind('click');
			this.oApi._fnSortAttachListener( oSettings, oSettings.aoColumns[i].nTh.children[0], i );
		}
		else{
...
ColReorder = function( oDTSettings, oOpts )
{
	...
	this.s = {
		...
		"aoTargets": [],
		/**
		 * for restor the each th element's original width value
		 */
		"originalTheadrWidths":null
	};
...
ColReorder.prototype = {
	...
	"_fnMouseDown": function ( e, nTh, i )
	{
	    ...
	    else if ($(nTh).css('cursor') == 'col-resize') {         
		...
		if (this.dom.resize) {       
		  ...
		  if($('div.dataTables_scrollBody') != null){
			...
			if($('div.dataTables_scrollBody').length > 0){
				...
				//Resize the columns
				if (moveLength != 0 && !scrollXEnabled){
					$(scrollingTableHead.children[0].children[visibleColumnIndex+1]).width(this.s.mouse.nextStartWidth - moveLength);
				}
				//Resize the scroller body table's th column
				$(scrollingTableHead.children[0].children[visibleColumnIndex]).width(this.s.mouse.startWidth + moveLength);
				
				/*if(widthBeforResize == th.width()){
					console.log("It's min width");
				}*/
				//Resize the table too
				if(scrollXEnabled&&$(tableScroller)[0].childNodes[0]!=null){
	...
	"_fnMouseUp": function ( e, colResized)
	{
		...
		//Martin Marchetta
			...
			if(this.s.dt.aoColumns[colResized].sClass.indexOf("unsortable")==-1){
